...
// Smooth anchor scroll with cinematic easing (no flicker)
...
// Class-based background control (no flicker)
(function(){
  ...
  // Blur only when SHOWREEL is prominent
  if (show) {
    const ioBlur = new IntersectionObserver((ents)=>{
      ents.forEach(ent=>{
        if(ent.isIntersecting && ent.intersectionRatio > 0.5){
          body.classList.add('blur-on');
        } else {
          body.classList.remove('blur-on');
        }
      });
    }, {threshold:[0,0.5,1]});
    ioBlur.observe(show);
